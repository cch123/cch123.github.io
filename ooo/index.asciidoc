...
在说到 cache contention 时经常提到。对于 CPU 来说，从内存中读取数据的最小单位就是一行 cache line。
...
字面含义，多个核心共享相同的数据。需要用 mesi 协议不停地 invalidate 其它核心中的 cache line。

== 课后问题

为什么 sync.Map 官方拒绝加入 Len 方法
