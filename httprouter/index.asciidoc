...
Q. httprouter 是不是不支持 /:hashcode 这种路由？
A. 是支持的，必须以 / 开头才行

Q. 这些路由的 zero garbage 和 0 alloc 是咋实现的？
A. zero garbage 指进程不在堆上分配内存。http router 实现了全静态路由时，匹配过程 0 alloc，这里的 0 次分配其实就是没有堆内存分配(栈上的肯定还是有的，但栈上分配内存不影响 GC)。
